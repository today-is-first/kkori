...
새로고침과 같은 **언마운트/리마운트 과정에서 사용자간 미디어 스트림 연결이 끊기는 문제**가 발생했습니다.
또한, **네트워크 변동 시 기존 ICE Candidate이 무효화되어 연결이 끊기게 되었습니다.**

원인을 분석하여 기존 PeerConnection 및 MediaStream 객체가 **브라우저 컨텍스트에서 소멸되어, 재마운트 시 새로운 객체 생성으로 기존 연결이 유지되지 않는 게 문제**라는 것을 파악했습니다.

미디어 스트림을 관리하는 스토어에 **비동기 작업을 공유하는 로직을 추가하고, deferred promise 패턴을 사용하여 해결**했습니다.

결과적으로, **리렌더링/재마운트 상황에서도 중복 초기화 및 Race Condition 상황을 제거하여 일관된 초기화를 보장**했습니다.

[Wiki로 자세히 보기](https://github.com/today-is-first/kkori/wiki/%EC%96%B8%EB%A7%88%EC%9A%B4%ED%8A%B8-%EB%A6%AC%EB%A7%88%EC%9A%B4%ED%8A%B8-%EC%8B%9C-%EB%AF%B8%EB%94%94%EC%96%B4-%EC%8A%A4%ED%8A%B8%EB%A6%BC-%EB%81%8A%EA%B9%80-%ED%98%84%EC%83%81-%ED%95%B4%EA%B2%B0)

...
새로고침과 같은 언마운트/리마운트 과정에서 사용자간 미디어 스트림 연결이 끊기는 문제가 발생했습니다.
또한, 네트워크 변동 시 기존 ICE Candidate이 무효화되어 연결이 끊기게 되었습니다.

원인을 분석하여 기존 PeerConnection 및 MediaStream 객체가 브라우저 컨텍스트에서 소멸되어, 재마운트 시 새로운 객체 생성으로 기존 연결이 유지되지 않는 게 문제라는 것을 파악했습니다.

미디어 스트림을 관리하는 스토어에 비동기 작업을 공유하는 로직을 추가하고, deferred promise 패턴을 사용하여 해결했습니다.

결과적으로, 리렌더링/재마운트 상황에서도 중복 초기화 및 Race Condition 상황을 제거하여 일관된 초기화를 보장했습니다.
